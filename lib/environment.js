...
function Environment(envId, creator, service, workspace) {
    _.bindAll(this);
    ...
    this.doc = new Document(null, creator.userId, this.service, this.transferPatch, this.workspace);
}
...
function createEnvironment(path, user, service, workspace) {
    var env = new Environment(path, user, service, workspace);
    return env.setup();
}
