...
Manager.prototype.createEnvironment = function(environmentId, user, workspace) {
    var that = this;
    return createEnvironment(environmentId, user, this.service, workspace).then(function(env) {
...
Manager.prototype.getEnvironment = function(environmentId, user, workspace) {
    if(!this.hasEnvironment(environmentId)) {
        return this.createEnvironment(environmentId, user, workspace);
    }
    return this._getEnvironment(environmentId);
};

Manager.prototype.handle = function(socket, workspace, data) {
    // Must be a valid request
    if(!v.base(data)) return;

    // Extract info
    var environmentId = workspace.identity() + '#' + data.environment;
    ...
    .then(function(user) {
        ...
        return that.getEnvironment(environmentId, user, workspace)
        .then(function(env) {
            // Do action
